...
final class GraphQLEndpoint
{
    ...
    {
        return self::executeQueryInternal($query, $variables);
    }
     ...
     * @param GraphQLQuery $query The GraphQL query to be executed.
     * @param array $variables (optional) A list of variables used by the GraphQL query. The list gets merged with the predefined list of variables from the given query.
     * @param int $cacheDuration (optional) Cache duration in seconds (default: 24 hours).
     * @return object The data of the query result as an appropriate PHP type.
     * @throws GraphQLException When the query result contains errors or the response content is invalid.
     */
    public function executeQueryWithCache(GraphQLQuery $query, array $variables = [], $cacheDuration = 86400): object
    {
        return self::executeQueryInternal($query, $variables, RssBridge::getCache(), $cacheDuration);
    }

    /**
     * Internal function for executeQuery and executeQueryWithCache.
     */
    private function executeQueryInternal(GraphQLQuery $query, array $variables = [], CacheInterface $cache = null, $cacheDuration = 86400): object
    {
        $queryData = $query->build($variables);
        $queryJson = json_encode($queryData);

        $cacheKey = isset($cache) ? sprintf('GraphQLQuery_%s_%s_%s', $this->url, $query->getName(), md5($queryJson)) : null;

        $result = null;

        if (isset($cacheKey)) {
            $resultJson = $cache->get($cacheKey);
        ...
        if (!isset($result)) {
            ...
            if (isset($cacheKey)) {
                $cache->set($cacheKey, $resultJson, $cacheDuration);
            }
        }
